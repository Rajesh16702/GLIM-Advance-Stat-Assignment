...
plot(pc5,type="lines") # scree plot 
#pc5$scores # the principal components
biplot(pc5)
load <- pc5$loadings[,1:2]
plot(load,type="n")
text(load,labels=names(ad1),cex=.7) # add variable names
...
autoplot(prcomp(carData), data = carData1, colour = 'ad1', loadings=TRUE, loadings.colour = 'blue',)

ef<-(tapply(carData1$TC1,list(carData1$ad1), mean))
fd<-(tapply(carData1$TC2,list(carData1$ad1), mean))

meanvector<-cbind(ad2,ef,fd)
